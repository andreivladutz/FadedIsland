/** 
 	The timer class is a tiny utility that computes the time for the Animator class
	
	It should be instantiated right before starting the Animator as it takes the time
	of the instantiation as the moment of the last update.
	
	The animator receives a Timer object as the parameter to the update function and uses
	the getDeltaTime() method to get the difference in time since the last update of the animation
 */
class Timer {
	constructor() {
	...
	getDeltaTime() {
		// first getDeltaTime() call inits the timeNow which is resetted only when lastUpdatedNow() is called
		if (!this._timeNow) {
	...
	// THIS METHOD SHOULD BE CALLED AFTER THE UPDATE OF ALL THE CURRENT ANIMATIONS
	lastUpdatedNow() {
		this._lastUpdateTime = (new Date()).getTime();
...
/**
 *	THIS CLASS CODE IS TAKEN FROM JURIY BURA'S PRO ANDROID WEB GAME APPS BOOK 
 *	WE DO NOT OWN ANY OF THE CODE FOR THE ANIMATOR CLASS EXCEPT FOR THE CHANGE IN THE	
 *	UPDATE FUNCTION WHICH NOW TAKES A TIMER INSTANCE AND NOT THE DELTA TIME
 *
 ...
 * of the animator.
 * @param timer - get time passed since the last upate using getDeltaTime() method. 0 is valid value.
 */
_p.update = function(timer) {
